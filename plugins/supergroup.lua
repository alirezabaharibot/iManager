...
local function check_member_super(cb_extra, success, result)
  ...
  if success == 0 then
	send_large_msg(receiver, "ابتدا روبات را ادمین کنید")
  end
...
local function lock_group_all(msg, data, target)
  ...
  if group_all_lock == 'yes' then
    return '🔐 تمام تنظیمات قفل است'
  else
    data[tostring(target)]['settings']['all'] = 'yes'
    save_data(_config.moderation.data, data)
    return '🔐 تمام تنظیمات قفل شد'
  end
end
...
local function unlock_group_all(msg, data, target)
  ...
  if group_all_lock == 'no' then
    return '🔓 قفل تمام تنظیمات باز است'
  else
    data[tostring(target)]['settings']['all'] = 'no'
    save_data(_config.moderation.data, data)
    return '🔓 قفل تمام تنظیمات باز شد'
  end
end
...
local function unlock_group_etehad(msg, data, target)
  ...
  if group_etehad_lock == 'no' then
    return '🔓 قفل اتحاد باز است'
  else
    data[tostring(target)]['settings']['etehad'] = 'no'
    save_data(_config.moderation.data, data)
    return '🔓 قفل اتحاد باز شد'
  end
end
...
local function lock_group_media(msg, data, target)
  ...
  if group_media_lock == 'yes' then
    return '🔐 قفل مدیا فعال بوده است'
  else
    data[tostring(target)]['settings']['media'] = 'yes'
    save_data(_config.moderation.data, data)
    return '🔐 قفل مدیا فعال شد'
  end
end
...
local function unlock_group_fosh(msg, data, target)
  ...
  if group_fosh_lock == 'no' then
    return '🔓 فحاشی در گروه ممنوع نیست'
  else
    data[tostring(target)]['settings']['fosh'] = 'no'
    save_data(_config.moderation.data, data)
    return '🔓 فحاشی در گروه آزاد شد'
  end
end
...
local function unlock_group_join(msg, data, target)
  ...
  if group_join_lock == 'no' then
    return '🔓 ورود به گروه قفل نیست'
  else
    data[tostring(target)]['settings']['join'] = 'no'
    save_data(_config.moderation.data, data)
    return '🔓 قفل ورود به گروه باز شد'
  end
end
...
local function lock_group_english(msg, data, target)
  ...
  if group_english_lock == 'yes' then
    return '🔐 چت انگلیسی قفل بوده است'
  else
    data[tostring(target)]['settings']['english'] = 'yes'
    save_data(_config.moderation.data, data)
    return '🔐 چت انگلیسی قفل شد'
  end
end
...
local function unlock_group_all(msg, data, target)
  ...
  local group_all_lock = data[tostring(target)]['settings']['all']
  if group_all_lock == 'no' then
    return '🔓 تمام تنظیمات قفل نیستند'
  else
    data[tostring(target)]['settings']['all'] = 'no'
    save_data(_config.moderation.data, data)
    return '🔓 قفل تمام تنظیمات باز شد'
...
local function lock_group_spam(msg, data, target)
  ...
  if not is_owner(msg) then
    return "👤 تنها مدیران دسترسی دارند"
  end
  local group_spam_lock = data[tostring(target)]['settings']['lock_spam']
  if group_spam_lock == 'yes' then
    return '🔐 اسپم گروه قفل بوده است'
  else
...
local function lock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'yes' then
    return '🔐 چت فارسی قفل بوده است'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'yes'
    save_data(_config.moderation.data, data)
    return '🔐 چت فارسی قفل شد'
  end
end
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return '🔓 چت فارسی از قبل باز بوده'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'no'
    save_data(_config.moderation.data, data)
    return '🔓 چت فارسی آزاد شد'
  end
end
...
local function lock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'yes' then
    return '🔐 قفل آر تی ال فعال بوده است'
  else
...
local function unlock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'no' then
    return '🔓 آر تی ال از قبل غیر فعال بوده'
  else
...
local function lock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'yes' then
    return '🔐 سختگیری قفل بوده است'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'yes'
    save_data(_config.moderation.data, data)
    return '🔐 سخت گیری قفل شد'
  end
end
...
local function unlock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'no' then
    return '🔓 قفل سخت گیری باز بوده است'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'no'
    save_data(_config.moderation.data, data)
    return '🔓 قفل سخت گیری باز شد'
  end
end
...
local function disable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'no' then
    return '🔓 قفل سختگیری باز است'
  else
    data[tostring(target)]['settings']['strict'] = 'no'
    save_data(_config.moderation.data, data)
    return '🔓 سختگیری غیرفعال شد'
  end
end
...
local function get_rules(msg, data)
  ...
  if not data[tostring(msg.to.id)][data_cat] then
    return '📄 قانونی تعیین نشده'
  end
  local rules = data[tostring(msg.to.id)][data_cat]
  local group_name = data[tostring(msg.to.id)]['settings']['set_name']
  local rules = group_name..'📄 قونین گروه : /n n/'..rules:gsub("/n", " ")
  return rules
end
...
local function promote2(receiver, member_username, user_id)
  ...
  data[group]['moderators'][tostring(user_id)] = member_tag_username
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username..' ارتقاع پیدا کرد')
end
...
local function demote2(receiver, member_username, user_id)
  ...
  data[group]['moderators'][tostring(user_id)] = nil
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username..' از مقام خود عزل شد ')
end
...
function get_message_callback(extra, success, result)
	...
	elseif get_cmd == "setowner" then
		...
		local channel_id = 'channel#id'..result.to.peer_id
			...
			if result.from.username then
				text = "@"..result.from.username.." [ "..result.from.peer_id.." ] آونر شد "
			else
				text = "[ "..result.from.peer_id.." ] آونر شد "
			end
			send_large_msg(channel_id, text)
		end
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'link' then
			...
			if not group_link then
				return "▫️ابتدا دستور /newlink را برای ساخت لینک ارسال کنید و سپس با دستور /link لینک گروه را دریافت کنید \n▫️اگر روبات سازنده گروه نیست میتوانید با دستور /setlink لینک گروه را تنظیم کنید"
			end
		...
		if matches[1] == 'promote' then
		  if not is_momod(msg) then
			...
			if not is_owner(msg) then
				return "⚠️ تنها مدیران / آونر ها میتوانند"
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'promote',
		...
		if matches[1] == 'setphoto' and is_momod(msg) then
			...
			return '🖼 لطفا تصویر جدید را ارسال نمایید'
		end
		...
		if matches[1] == 'help' and not is_owner(msg) then
			text = "برای مشاهده راهنمای روبات به پیوی روبات مراجعه نمایید"
